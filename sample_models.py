...
def final_model(input_dim, recur_layers, dropout_rate,pool_size, filters, kernel_size, conv_stride,conv_border_mode, units, output_dim=29):
    ...
    for layer in range(recur_layers):
        if layer == 0:
            ...
            # Add batch normalization
            bn_cnn = BatchNormalization(name='bn_conv_1d')(conv_1d)
            drop_out_cnn = Dropout(rate=dropout_rate)(bn_cnn)
            pool_cnn = MaxPooling1D(pool_size, name=f'cnn_max_pool_{layer+1}')(drop_out_cnn)
            
            bidir_rnn = Bidirectional(GRU(units, activation=activation,
                        return_sequences=True, implementation=2, name='birnn'+str(layer)),merge_mode='concat')(pool_cnn)
            bn_rnn = BatchNormalization()(bidir_rnn)
        else:
            bidir_rnn = Bidirectional(GRU(units, activation=activation,
                        return_sequences=True, implementation=2, name='birnn'+str(layer)),merge_mode='concat')(bn_rnn)
            bn_rnn = BatchNormalization()(bidir_rnn)
            activation_layer = Activation(activation)(bn_rnn)
            drop_out_rnn = Dropout(rate=dropout_rate)(activation_layer)
